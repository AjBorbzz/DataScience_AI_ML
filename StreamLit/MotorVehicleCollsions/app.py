...
file = 'Motor_Vehicle_Collisions_slim_version.csv'
...
def load_data(n_rows):
    data = pd.read_csv(file, nrows=n_rows, parse_dates=[['CRASH_DATE', 'CRASH_TIME']])
...
st.map(data.query("injured_persons >= @injured_people")[["latitue", "longitude"]].dropna(how="any"))

st.header("How many collisions occure during a given time of a day?")
...
hist = np.histogram(filtered['date/time'].dt.minute, bins60, range=(0,60))[0]
...
st.header("Top 5 dangerous street")
...
if select == 'Pedestrians':
    st.write(original_data
        ...
        .sort_value(by=['injured_pedestrians'], ascending=False)
        .dropna(how='any'))[:5]

elif select == 'Cyclists':
    st.write(original_data
        .query("injured_cyclists" >= 1)[["on_street_name", "injured_cyclists"]]
        .sort_value(by=['injured_cyclists'], ascending=False)
        .dropna(how='any'))[:5]

elif select == 'Motorists':
    st.write(original_data
        .query("injured_motorists" >= 1)[["on_street_name", "injured_motorists"]]
        .sort_value(by=['injured_motorists'], ascending=False)
        .dropna(how='any'))[:5]
